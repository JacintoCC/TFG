...
/**
 * The Daniel Trend test.
 * 
 * This test computes Spearman's correlation coefficient to
...
public class DanielTrendTest extends StatisticalTest{
	 ...
	 */
	private SpearmanDistribution distribution;
	...
	/**
	 * R statistic (Spearman's correlation coefficient)
	 */
	private double R;
	...
	public DanielTrendTest(){
		...
		distribution=SpearmanDistribution.getInstance();
		setReportFormat();
		clearData();
	...
	public void doTest(){
		...
		for(int i=0;i<samples.length;i++){
			D=samples[i].a-samples[i].b;
			sumD+=(D*D);
		}

		//Compute R
		R=1.0-((6.0*sumD)/(double)(n*((n*n)-1.0)));

		//compute exact p-value
		if((n>2)&&(n<11)){
			exactPValue=distribution.computeExactProbability((int)n,R);
		}
		else{
			if((n>10)&&(n<31)){
				exactPValue=distribution.computeAproximatedProbability((int)n,R);
			}	
		}
	...
	public String printReport(){
		...
		report+="\n*************\n";
		report+="Daniel Trend Test (Spearman correlation between bivariate samples.)\n";
		report+="*************\n\n";
