...
$\bar{X}_1, \bar{X}_2$ a las medias de las dos medias. Las 
...
El test se realiza utilizando el estadístico $t$
	\[
		t = \frac{\bar{X}_1 - \bar{X}_2}
				{\sqrt{\frac{\tilde{s}_1^2}{n_1} + 
				       \frac{\tilde{s}_2^2}{n_2}}},
	\]
	
	donde $n_1, n_2$ son los datos de cada muestra (en 
nuestro caso tendremos $n_1 = n_2 = N$). $\bar{X}_i, i=1,2$ 
es la media muestral $\sum\limits_{j=1}^{n_i} X_{ij}$ de los 
valores obtenidos en cada muestra y $\tilde{s}_i$ es el 
estimador para la varianza
	\[
		\tilde{s}_i^2 = 
			\frac{\sum\limits_{j=1}^{n_i} X_{ij}^2 -
				  \frac{\left(
				  			\sum\limits_{j=1}^{n_i} X_{ij}
				  		\right)^2}
				  		{n_i}}
				{n_i - 1}.
	\]
...
para la distribución $t$ de Student con $n_1 + n_2 - 2$ 
grados de libertad.\\
	Un más detallado análisis de la aplicación del $t$-test
en la comparación del rendimiento en aprendizaje automático
se efectúa en el capítulo siguiente.
...
clasificadores, esto plantea los problemas de que son 
necesarios numerosos test para realizar todas las 
combinaciones y por tanto la información que se obtiene es
más difícil de analizar, y el otro problema es el incremento
de la probabilidad de cometer un error de tipo I. Presentamos
en esta sección la versión paramétrica, ANOVA.\\
...
los errores de un clasificador en todas las bases de datos):
	\begin{enumerate}
	\item variación en un grupo,
	\item variación entre grupos, y
	\item variación total, como suma de las dos anteriores.
	\end{enumerate}

	Consideraremos para realizar este test que disponemos de 
$n$ bases de datos y $k$ clasificadores. Notaremos por 
...
bases de datos. Los datos se presentan en una tabla $n \times 
...
corresponden con bases de datos y que las columnas 
...
nuestro problema esto significa que las bases de datos sí son 
independientes pero cada algoritmo los resultados guardan 
...
el Teorema Central del Límite Permutacional (\ref{th:PCTL}) para 
aproximar la distribución de la permutación 
$F[t|\mathbf{X}]$. Para resolver un problema de este tipo 
podemos guiarnos por lo siguiente:
	
\begin{enumerate}[a]
	\item Si $n \leq 25$ es posible calcular exactamente $T*$ 
		para todas las posibles permutaciones y calcular 
		$F[t|\mathbf{X}]$.
...
función no degenerada $T: \Omega^n \rightarrow \mathbf{R}$. 
...
\mathbf{X}}$. Esto implica que para $\alpha$ fijo, $T_\alpha$ 
...
\end{align*}
	...
	sabemos que la cdf de $Z$, $F_Z(t|\Omega^n)$ puede 
...
sus funciones de potencia. Suponemos $X \leadsto F(\theta_X)
$, $Y \leadsto F(\theta_Y)$. Consideraremos la hipótesis nula 
...
varias bases de datos, evaluando sus ventajas e 
inconvenientes:
	\begin{enumerate}
...
algoritmos una vez rechazada la hipótesis nula. 
	\begin{itemize}
	\item Procedimiento de Nemenyi: Se ajusta el valor de 
		$\alpha$ dividiendo por el número de comparaciones 
		$\frac{k(k-1)}{2}$. 
	\item Procedimiento de Holm: Ajusta el valor de $\alpha$ 
		de manera descendente. Siendo $p_1, \dots, p_m$ los 
		$p$-valores ordenados de menor a mayor y $H_1, \dots, 
		H_m$ sus correspondientes hipótesis, se rechazan 
		$H_1, \dots, H_{i-1}$ para $i$ el menor número tal 
		que $p_i > \alpha/(m-i+1)$
	\end{itemize}
	
	Debido a que las posibles hipótesis sobre la comparación 
por parejas están relacionadas y no se pueden dar todas las 
posibles combinaciones, se presentan los procedimientos de 
...
sobre treinta bases de datos se observa como el procedimiento 
de Bergmann y Hommel tiene una mayor potencia.\\
...
Pearson) y el test de Levene de Heterocedasticidad. Se 
...
heterocedasticidad para comprobar si se dan las condiciones 
...
bases de datos como estimador, utilizando el algoritmo ~
...
las bases de datos y \textit{fold}, se encuentra en las colas 
...
en 32 bases de datos para una secuencia de diferencias en sus 
...
\subsubsection{t-test bayesiano correlado}
